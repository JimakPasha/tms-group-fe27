...
const arrayTodo = ['Clean room', 'Do homework', 'learn js'];
...
for (let i = 0; i < arrayTodo.length; i++) {
    ...
    todoLabelSpan.style.backgroundColor = 'red';
...
function checkCheckbox() {
    for (let i = 0; i < arrayTodo.length; i++) {
        let fx = document.getElementsByClassName('todoCheckbox');
        let labTest = document.getElementsByClassName('todoSpan');
        if (fx[i].checked) {
            labTest[i].style.backgroundColor = 'aquamarine';
            labTest[i].innerHTML = '&#10003';
        } else {
            labTest[i].style.backgroundColor = 'red';
            labTest[i].innerHTML = '';
        }
    }
